...
namespace Extensions
{
	...
	{
		public static SimpleCoroutineAwaiter<T> AsyncRun<T>(this MonoBehaviour mono, IEnumerator<T> enumerator)
		{
			var awaiter = new SimpleCoroutineAwaiter<T>();
			mono.StartCoroutine(new CoroutineWrapper<T>(enumerator, awaiter).Run());
			return awaiter;
		}
		public static SimpleCoroutineAwaiter<object> AsyncRun(this MonoBehaviour mono, IEnumerator enumerator)
		{
			var awaiter = new SimpleCoroutineAwaiter<object>();
			mono.StartCoroutine(new CoroutineWrapper<object>(enumerator, awaiter).Run());
			return awaiter;
		}
		public static SimpleCoroutineAwaiter<object> AsyncPlay(this MonoBehaviour mono, Animation animation, string clip = null)
		{
			if (clip == null)
			{
				clip = animation.clip.name;
			}
			IEnumerator animationIEnumerator()
			{
				var preIsplaying = false;
				while (true)
				{
					if (animation.isPlaying == false && preIsplaying == true || animation[clip].normalizedTime == 1)
					{
						yield break;
					}
					preIsplaying = animation.isPlaying;
					yield return null;
				}
			}
			var awaiter = new SimpleCoroutineAwaiter<object>();
			mono.StartCoroutine(new CoroutineWrapper<object>(animationIEnumerator(), awaiter).Run());
			animation.Play(clip);
			return awaiter;
		}
	}
	...
	public class SimpleCoroutineAwaiter<T> : INotifyCompletion
	{
		...
		public SimpleCoroutineAwaiter<T> GetAwaiter()
		{
			return this;
		}
	}
